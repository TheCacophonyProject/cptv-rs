...
class CPTVWriter:
    ...
    def write_header(self):
        ...
        self.s = gzip.GzipFile(fileobj=self.fileobj, mode="wb", mtime=mtime, compresslevel=1)
        self.comp = CompressorRs()
    ...
    def write_frame(self, frame):
        # Rust code call-out here?
        tic = time.perf_counter()
        bit_width, frame_buf = self.comp._next_frame(frame.pix)
...
class CompressorRs:
    ...
    def _next_frame(self, pix):
        if self.prev is not None:
            packed = encoder_py_bindings.push_frame_data(self.prev.tobytes(), pix.tobytes(), 160, 120)
        else:
            packed = encoder_py_bindings.push_frame_data(None, pix.tobytes(), 160, 120)
        self.prev = pix
        return 16, packed
